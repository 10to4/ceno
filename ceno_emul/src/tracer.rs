...
impl StepRecord {
    ...
    pub fn new_u_instruction(
        ...
        let pc = Change::new(pc, pc + PC_STEP_SIZE);
        StepRecord::new_insn(cycle, pc, insn, None, None, Some(rd), None, prev_cycle)
    }
    ...
    fn new_insn(
        cycle: Cycle,
        pc: Change<ByteAddr>,
        // TODO: review whether we actually need to provide the full instruction, instead of just the kind for all the supplying functions.
        insn: Instruction,
...
impl Tracer {
    ...
    pub fn fetch(&mut self, pc: WordAddr, value: Instruction) {
        ...
        // TODO(Matthias): we should probably store the decoded instruction
