    ...
    fn run_maybe_challenge(
        ...
        if let Some(lkm_from_assignment) = lkm {
            ...
            let lkm_from_cs = {
                ...
                result.into_iter().take(instances - 1)
            }
            .map(|records| {
                records
                    .iter()
            ...
            // Compare LK Multiplicity from constraint system and assignment for every instance, record the errors if any.
            for (inst_id, (lkm_from_cs, lkm_from_assignment)) in lkm_from_cs
                .into_iter()
                .zip_eq(lkm_from_assignment.into_iter())
                .enumerate()
            {
                compare_lkm(inst_id, lkm_from_cs, lkm_from_assignment, &mut errors);
            }
        }
...
fn transpose<E: ExtensionField>(
    ...
    let mut result: Vec<Vec<(ROMType, Vec<u64>)>> = vec![];

    for (record_id, (record_rom, record_args)) in data.iter().enumerate() {
        for (arg_id, (_expr, instance_vec)) in record_args.iter().enumerate() {
            for (inst_id, value) in instance_vec.iter().enumerate() {
                if result.len() <= inst_id {
                    result.push(vec![]);
                }
                if result[inst_id].len() <= record_id {
                    result[inst_id].push((*record_rom, vec![]));
                }
                if result[inst_id][record_id].1.len() <= arg_id {
                    result[inst_id][record_id].1.push(*value);
                }
            }
        }
    }
...
fn compare_lkm<E: ExtensionField>(
    ...
    for (rom_type, cs_map, ass_map) in izip!(ROMType::iter(), &lkm_from_cs, &lkm_from_assignment) {
        if *cs_map != *ass_map {
